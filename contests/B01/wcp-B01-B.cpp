    ...
    void solve(int tc){ //use tc for debugging
        ...
        it(i,n){
            int x = a[i];
            if(i == 0 and x < 0) si = false;
            if((x>0 and si) or (x<0 and !si)) somes[len(somes)-1] += x;
            else{
                si = !si;
                somes.pb(x);
            }
        ...
        for(int ptr=start; ptr+1<len(somes); ptr += 2){
            if(somes[ptr] + somes[ptr+1] < 0) cns -= somes[ptr]+somes[ptr+1];
            else{
                if(ptr+2 < n){
                    somes[ptr+2] += somes[ptr]+somes[ptr+1];
                }
            }
        }
        cout << cns << "\n";
